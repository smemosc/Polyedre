...
class Polyedr:
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    # задание самой грани
                    fa = Facet(vertexes)
                    fb = Facet(stan_vertexes)
                    self.facets.append(fa)
                    if fb.center().x ** 2 + fb.center().y ** 2 + \
                            fb.center().z ** 2 < 4:
    ...
    def sum_perimeter(self):
        sum_per = 0
        for i in self.right_facets:
            f = True
            e = 0
            for n in range(len(i.vertexes)):
                a = Edge(i.vertexes[n-1], i.vertexes[n])
                e += sqrt(
                        (i.vertexes[n - 1].x - i.vertexes[
                            n].x) ** 2 +
                        (i.vertexes[n - 1].y - i.vertexes[
                            n].y) ** 2 +
                        (i.vertexes[n - 1].z - i.vertexes[
                            n].z) ** 2)
                for f in self.facets:
                    a.shadow(f)
                    if a.invisible:
